...
for i = 1:Nx, j = 1:Ny
    jacobian_approx[i, j] = ∂xᶠᶜᶜ(i, j, 1, grid, f, xᶜ, yᶜ) * ℑxyᶠᶜᵃ(i, j, 1, grid, ∂yᶜᶠᶜ, g, xᶜ, yᶜ)
                          - ∂xᶠᶜᶜ(i, j, 1, grid, g, xᶜ, yᶜ) * ℑxyᶠᶜᵃ(i, j, 1, grid, ∂yᶜᶠᶜ, f, xᶜ, yᶜ)
end

for i = 1:Nx, j = 1:Ny
    jacobian_exact[i, j] = ∂f_x(i, j, 1, grid, xᶠ, yᶜ) * ∂g_y(i, j, 1, grid, xᶠ, yᶜ) 
                         - ∂f_y(i, j, 1, grid, xᶠ, yᶜ) * ∂g_x(i, j, 1, grid, xᶠ, yᶜ)
end
