...
function jacobian_x(i, j, k, grid, A, h)
    return ((∂xᶠᶜᶜ(i, j, 1, grid, A)*ℑxyᶠᶜᵃ(i, j, k, grid, ∂yᶜᶠᶜ, Bx, A, h)) 
            - (ℑxyᶠᶜᵃ(i, j, 1, grid, ∂yᶜᶠᶜ, A)*∂xᶠᶜᶜ(i, j, k, grid, Bx, A, h)))
end

function jacobian_y(i, j, k, grid, A, h)
    return ((ℑxyᶜᶠᵃ(i, j, k, grid, ∂xᶠᶜᶜ, A)*∂yᶜᶠᶜ(i, j, k, grid, By, A, h))
            - (∂yᶜᶠᶜ(i, j, k, grid, A)*ℑxyᶜᶠᵃ(i, j, k, grid, ∂xᶠᶜᶜ, By, A, h)))
end
