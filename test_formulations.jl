...
function By(i, j, k, grid, A, args...)
    return ℑxᶜᵃᵃ(i, j, k, grid, ∂xᶠᶜᶜ, A, args...)
end

function Bx(i, j, k, grid, A, args...)
    return -ℑyᵃᶜᵃ(i, j, k, grid, ∂yᶜᶠᶜ, A, args...)
end


# Jacobian formulation of lorentz force

function jacobian_x(i, j, k, grid, A, args...)
    return ((∂xᶠᶜᶜ(i, j, 1, grid, A, args...) *ℑxyᶠᶜᵃ(i, j, k, grid, ∂yᶜᶠᶜ, Bx, A, args...)) 
            - (ℑxyᶠᶜᵃ(i, j, 1, grid, ∂yᶜᶠᶜ, A, args...)*∂xᶠᶜᶜ(i, j, k, grid, Bx, A, args...)))
end

function jacobian_y(i, j, k, grid, A, args...)
    return ((ℑxyᶜᶠᵃ(i, j, k, grid, ∂xᶠᶜᶜ, A, args...)*∂yᶜᶠᶜ(i, j, k, grid, By, A, args...))
            - (∂yᶜᶠᶜ(i, j, k, grid, A, args...)*ℑxyᶜᶠᵃ(i, j, k, grid, ∂xᶠᶜᶜ, By, A, args...)))
end

function lorentz_force_func_x(i, j, k, grid, A, args...)
    return (jacobian_x(i, j, k, grid, A, args...))
end

function lorentz_force_func_y(i, j, k, grid, A, args...)
    return (jacobian_y(i, j, k, grid, A, args...))
end
...
function _Bx(i, j, k, grid, A, args...)
    return -ℑxyᶠᶜᵃ(i, j, k, grid, ∂yᶜᶠᶜ, A, args...)
end

function _By(i, j, k, grid, A, args...)
    return ℑxyᶜᶠᵃ(i, j, k, grid, ∂xᶠᶜᶜ, A, args...)
end

function _hBx(i, j, k, grid, A, args...)
    return -ℑxyᶠᶜᵃ(i, j, k, grid, ∂yᶜᶠᶜ, A, args...)
end

function _hBy(i, j, k, grid, A, args...)
    return ℑxyᶜᶠᵃ(i, j, k, grid, ∂xᶠᶜᶜ, A, args...)
end


@inline lorentz_flux_hBx_bx(i, j, k, grid, A, args...) = 
    @inbounds advective_lorentz_flux_hBx_bx(i, j, k, grid, _hBx, _Bx, A, args...)

@inline lorentz_flux_hBy_bx(i, j, k, grid, A, args...) =
    @inbounds advective_lorentz_flux_hBy_bx(i, j, k, grid, _hBy, _Bx, A, args...)

@inline lorentz_flux_hBx_by(i, j, k, grid, A, args...) =
    @inbounds advective_lorentz_flux_hBx_by(i, j, k, grid, _hBx, _By, A, args...)

@inline lorentz_flux_hBy_by(i, j, k, grid, A, args...) =
    @inbounds advective_lorentz_flux_hBy_by(i, j, k, grid, _hBy, _By, A, args...)
...
for N in Ns
    ...
    local jacobian_lorentz_x = zeros(Float64, length(xᶜ), length(yᶜ))
    local jacobian_lorentz_y = zeros(Float64, length(xᶜ), length(yᶜ))
    ...
    for i = 1:Nx, j = 1:Ny
        ...
        jacobian_lorentz_x[i, j] = lorentz_force_func_x(i, j, 1, grid, A, xᶜ, yᶜ)
        jacobian_lorentz_y[i, j] = lorentz_force_func_y(i, j, 1, grid, A, xᶜ, yᶜ)

        error_x[i, j] = maximum(abs(jacobian_lorentz_x[i, j] - exact_lorentz_x[i, j]))
        error_y[i, j] = maximum(abs(jacobian_lorentz_y[i, j] - exact_lorentz_y[i, j]))
    
    end

    error_lorentz_x[N] = maximum(abs, jacobian_lorentz_x .- exact_lorentz_x)
    error_lorentz_y[N] = maximum(abs, jacobian_lorentz_y .- exact_lorentz_y)
